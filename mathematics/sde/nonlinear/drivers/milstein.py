...
def milstein(y0: np.array, a: Matrix, b: Matrix, k: float, times: tuple):
    ...
    logger.info(f"[{(time() - start_time):.3f} seconds] Milstein start")
    ...
    logger.info(f"[{(time() - start_time):.3f} seconds] Milstein subs are finished")
    ...
    logger.info(f"[{(time() - start_time):.3f} seconds] Milstein calculations are finished")

...
def strong_taylor_ito_2p0(y0: np.array, a: Matrix, b: Matrix, k: float, times: tuple):
    ...
    logger.info(f"[{(time() - start_time):.3f} seconds] Strong Taylor-Ito 2.0 start")
    ...
    logger.info(f"[{(time() - start_time):.3f} seconds] Strong Taylor-Ito 2.0 subs are finished")
    ...
    logger.info(f"[{(time() - start_time):.3f} seconds] Strong Taylor-Ito 2.0 calculations are finished")
